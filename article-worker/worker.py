"""
Article Worker - Temporal Python Worker

Executes ArticleCreationWorkflow for comprehensive article generation.
"""

import asyncio
import os
import sys

from temporalio.client import Client
from temporalio.worker import Worker
from dotenv import load_dotenv

# Load environment variables
load_dotenv()

# Import workflow
from src.workflows.article_creation import ArticleCreationWorkflow

# Import all activities
from src.activities.normalize import (
    normalize_article_topic,
    check_article_exists,
)

from src.activities.research.serper import (
    fetch_topic_news,
)

from src.activities.research.exa import (
    exa_research_topic,
)

from src.activities.research.crawl_news import (
    crawl_news_sources,
)

from src.activities.research.crawl_auth import (
    crawl_authoritative_sites,
)

from src.activities.validation.link_validator import (
    playwright_url_cleanse,
    playwright_clean_article_links,
)

from src.activities.generation.content_generation import (
    generate_article_content,
)

from src.activities.generation.image_generation import (
    generate_article_contextual_images,
)

from src.activities.articles.analyze_sections import (
    analyze_article_sections,
)

from src.activities.articles.company_extraction import (
    extract_company_mentions,
    calculate_article_completeness,
)

from src.activities.storage.neon_database import (
    save_article_to_neon,
    link_companies_to_article,
)

from src.activities.storage.zep_integration import (
    query_zep_for_article_context,
    sync_article_to_zep,
    create_article_zep_summary,
)

from src.utils.config import config


async def main():
    """Start the Temporal worker"""

    print("=" * 70)
    print("📝 Article Worker - Starting...")
    print("=" * 70)

    # Display configuration
    print("\n🔧 Configuration:")
    print(f"   Temporal Address: {config.TEMPORAL_ADDRESS}")
    print(f"   Namespace: {config.TEMPORAL_NAMESPACE}")
    print(f"   Task Queue: {config.TEMPORAL_TASK_QUEUE}")
    print(f"   API Key: {'✅ Set' if config.TEMPORAL_API_KEY else '❌ Not set'}")
    print(f"   Environment: {config.ENVIRONMENT}")

    # Validate required environment variables
    missing = config.validate_required()

    if missing:
        print(f"\n❌ Missing required environment variables:")
        for var in missing:
            print(f"   - {var}")
        print("\n   Please set them in .env file or environment")
        sys.exit(1)

    print("\n✅ All required environment variables present")

    # Display service status
    print("\n📊 Service Status:")
    service_config = config.as_dict()
    for key, value in service_config.items():
        if key.startswith("has_"):
            service_name = key.replace("has_", "").upper()
            status = "✅" if value else "❌"
            print(f"   {status} {service_name}")

    # Connect to Temporal
    print(f"\n🔗 Connecting to Temporal Cloud...")

    try:
        if config.TEMPORAL_API_KEY:
            # Temporal Cloud with TLS
            client = await Client.connect(
                config.TEMPORAL_ADDRESS,
                namespace=config.TEMPORAL_NAMESPACE,
                api_key=config.TEMPORAL_API_KEY,
                tls=True,
            )
        else:
            # Local Temporal (development)
            client = await Client.connect(
                config.TEMPORAL_ADDRESS,
                namespace=config.TEMPORAL_NAMESPACE,
            )

        print("✅ Connected to Temporal successfully")

    except Exception as e:
        print(f"❌ Failed to connect to Temporal: {e}")
        sys.exit(1)

    # Create worker with all workflows and activities
    worker = Worker(
        client,
        task_queue=config.TEMPORAL_TASK_QUEUE,
        workflows=[ArticleCreationWorkflow],
        activities=[
            # Normalization
            normalize_article_topic,
            check_article_exists,

            # Research
            fetch_topic_news,
            exa_research_topic,
            crawl_news_sources,
            crawl_authoritative_sites,

            # Validation
            playwright_url_cleanse,
            playwright_clean_article_links,

            # Generation
            generate_article_content,
            generate_article_contextual_images,

            # Articles
            analyze_article_sections,
            extract_company_mentions,
            calculate_article_completeness,

            # Database
            save_article_to_neon,
            link_companies_to_article,

            # Zep Integration
            query_zep_for_article_context,
            sync_article_to_zep,
            create_article_zep_summary,
        ],
    )

    print("\n" + "=" * 70)
    print("🚀 Article Worker Started Successfully!")
    print("=" * 70)
    print(f"   Task Queue: {config.TEMPORAL_TASK_QUEUE}")
    print(f"   Environment: {config.ENVIRONMENT}")
    print("=" * 70)

    print("\n📋 Registered Workflows:")
    print("   - ArticleCreationWorkflow")

    print("\n📋 Registered Activities:")
    activity_groups = [
        ("Normalization", ["normalize_article_topic", "check_article_exists"]),
        ("Research", [
            "fetch_topic_news",
            "exa_research_topic",
            "crawl_news_sources",
            "crawl_authoritative_sites",
        ]),
        ("Validation", ["playwright_url_cleanse", "playwright_clean_article_links"]),
        ("Generation", [
            "generate_article_content",
            "generate_article_contextual_images",
        ]),
        ("Articles", [
            "analyze_article_sections",
            "extract_company_mentions",
            "calculate_article_completeness",
        ]),
        ("Database", [
            "save_article_to_neon",
            "link_companies_to_article",
        ]),
        ("Zep Integration", [
            "query_zep_for_article_context",
            "sync_article_to_zep",
            "create_article_zep_summary",
        ]),
    ]

    for group_name, activities in activity_groups:
        print(f"\n   {group_name}:")
        for activity in activities:
            print(f"     - {activity}")

    print("\n✅ Worker is ready to process article creation workflows")
    print("   Press Ctrl+C to stop\n")

    # Run worker (blocks until interrupted)
    await worker.run()


if __name__ == "__main__":
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
        print("\n\n👋 Article Worker stopped by user")
        sys.exit(0)
    except Exception as e:
        print(f"\n\n❌ Article Worker crashed: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
