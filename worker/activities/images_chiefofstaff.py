"""
Chief of Staff Image Generation Activity

Executive-focused, sophisticated imagery with "Chief of Staff" branding.
...
@activity.defn(name="generate_chiefofstaff_images")
async def generate_chiefofstaff_images(
    article_id: str,
    article_title: str,
    article_angle: str
) -> Dict[str, str]:
    """
    Generate 6 executive-focused images for Chief of Staff articles

    All images include "Chief of Staff" branding in the bottom left corner.
    ...
    """
    activity.logger.info(f"🎨 Generating CHIEF OF STAFF images: {article_title[:50]}")
    ...
    try:
        # Executive-focused prompts with "Chief of Staff" branding
        prompts = {
...
IMPORTANT: Include elegant text 'Chief of Staff' in bottom left corner in refined sans-serif font, white or light gray color""",
...
IMPORTANT: Include text 'Chief of Staff' in bottom left corner in professional font, clearly visible""",
...
IMPORTANT: Include text 'Chief of Staff' in bottom left corner, white text on subtle dark overlay""",
...
IMPORTANT: Include 'Chief of Staff' text in bottom left corner in elegant font""",
...
IMPORTANT: Include 'Chief of Staff' branding in bottom left corner in professional style""",
...
IMPORTANT: Include text 'Chief of Staff' in bottom left corner, clean professional typography"""
        }
            ...
            if url:
                try:
                    ...
                    result = await asyncio.to_thread(
                        ...
                        public_id=f"chiefofstaff_{purpose}_{article_id}",
                        overwrite=True,
                        resource_type="image"
                    )
        ...
        activity.logger.info(f"✅ Chief of Staff images complete: {len([u for u in cloudinary_urls.values() if u])}/6 successful")
        return cloudinary_urls

    except Exception as e:
        import traceback
        activity.logger.error(f"❌ Chief of Staff image generation failed: {e}")
