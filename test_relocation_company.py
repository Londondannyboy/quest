#!/usr/bin/env python3
"""
Test script for RelocationCompanyWorkflow

Creates a company profile for a relocation service provider.
"""
...
async def test_relocation_company():
    """Test the relocation company workflow"""
    ...
    print("🧪 Testing Relocation Company Workflow")
    print("=" * 60)
    ...
    # Test company - Santa Fe Relocation (well-known relocation services company)
    test_company = {
        "company_name": "Santa Fe Relocation",
        "company_website": "https://www.santaferelo.com",
    }
    ...
    # Start workflow
    workflow_id = f"relocation-company-test-{asyncio.get_event_loop().time()}"
    ...
    # Countries served
    countries = result.get('countries_served', [])
    if countries:
        print(f"Countries Served: {', '.join(countries[:5])}")
        if len(countries) > 5:
...
if __name__ == "__main__":
    try:
        asyncio.run(test_relocation_company())
